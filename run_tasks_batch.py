...
def get_toml_files_from_volume(output_directory):
    ...
    try:
        ...
        cmd = f'uv run modal volume ls robogen-generated_task_outputs {output_directory}'
...
def run_task_generation(task_description, model_provider, detach=True):
    ...
    cmd_parts.extend([
        ...
        '--target-model-provider', model_provider,
        '--generate-task'
    ])
...
def log_task_run(csv_path, task_description, model_name, model_provider, run_number,
                 status, app_id=None, output_directory=None, toml_files=None, notes=None):
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help='Delay in seconds between task submissions'
    )
    parser.add_argument(
        '--no-detach',
        action='store_true',
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help='Start from task number N (0-indexed)'
    )
    parser.add_argument(
        '--max-tasks',
        type=int,
